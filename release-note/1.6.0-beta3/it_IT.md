...
### Fixed
- Risolto il problema con il metodo per ottenere l'ID del dispositivo
- Risolto il problema per cui i dischi personalizzati non venivano visualizzati completamente
- Risolti diversi problemi di stratificazione dello z-index
- Risolto il problema per cui lo spazio di archiviazione appariva mancante a causa della presenza di dischi di tipo vfat
- Risolto il problema per cui la formattazione di un disco rigido abilitato non veniva salvata correttamente nel database
- Risolto il problema con la visualizzazione imprecisa del Cestino

### Improved
- Ottimizzata la logica per determinare i dischi membri RAID mancanti
- Ottimizzata la logica di salvataggio per i dischi personalizzati
...
### Fixed
- Risolto il problema per cui non era possibile ottenere probabilisticamente l'indice accurato del disco, risolvendo il problema dei dischi che occasionalmente scomparivano nella WebUI.
- Risolto il problema per cui i dispositivi USB con partizioni non potevano essere montati automaticamente.
- Risolto il problema per cui le immagini dei dispositivi personalizzati con sfondi trasparenti venivano visualizzate come nere.
...
- Ottimizzata la logica di visualizzazione del pulsante di abilitazione dello storage per facilitarne l'individuazione.
